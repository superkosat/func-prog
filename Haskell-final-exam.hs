...
--Note: Something peculiar happened here with tabulation when I attempted
--to load the file in ghci, only works with a single space. Never had that
--happen before
in3 :: ChessPos -> [ChessPos]
in3 startPos = do
 firstPos <- moveKnight startPos
 secondPos <- moveKnight firstPos
 moveKnight secondPos
...
in3' :: ChessPos -> [ChessPos]
in3' startPos =
  moveKnight startPos >>= \firstPos ->
    moveKnight firstPos >>= \secondPos ->
      moveKnight secondPos >>= \thirdPos ->
        return thirdPos
...
-- Give an elementary definition using pattern matching
-- of the pop and push functions

popS :: Stack -> (Int, Stack)
popS [] = error "empty stack"
popS (x:xs) = (x, xs)
...
instance Applicative (State s) where
  pure x = State (\s -> (x, s))
  (State k) <*> (State h) = State (\s -> let (f, s') = k s
                                             (a, s'') = h s'
                                         in (f a, s''))
...
popM = State (\stack -> case stack of
                          [] -> error "Stack is empty"
                          (x:xs) -> (x, xs))

pushM :: Int -> State Stack ()
pushM x = State (\stack -> ((), x:stack))
...
--The get function takes a State monad and returns its current state,
--while the the put function takes the State and updates it to a new one
...
look _ [] = Nothing
look variable ((var, val):rest)
  | variable == var = Just val
  | otherwise = look variable rest
...
cleanup :: Store -> Store
cleanup [] = []
cleanup (x:xs) = x : cleanup (filter (\(v, _) -> v /= fst x) xs)
...
-- assigned to a given variable in the list and to remove
-- the other (earlier) assignments of the variable.
-- The cleanup function enables us to think of a state
-- as a Map with strings as keys and integers as values.
...
exec (x := e) r                    = (x, eval e r) : cleanup (filter (\(v, _) -> v /= x) r)
